...
async function rateLimitedFetch(url, key, host) {
  ...
  console.log(`[API FETCH] ${url} ->`, JSON.stringify(json).substring(0, 300));
  return json;
}
...
function normalizeProfile(raw) {
  if (!raw) return null;
  // Handle nested structures common in GraphQL and Looter2 scrapers
  const data = raw.node || raw.user || raw;
